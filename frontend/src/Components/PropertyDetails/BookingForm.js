  ...
  const isDateDisabled = (current) =>{
    ...
    else
    {
      return disabledDates.some((date)=>{
        ...
        return(current.isBefore(Date.now(), "date")|| 
...
const handleBookPlace = (e) => {
  e.preventDefault()
  if(userData?.name && userData?.guests && userData?.phoneNo && paymentData?.checkinDate && paymentData?.checkoutDate)
  {
        ...
        <div className='payment-field'>
          <div className='date'>
            <Space direction='vertical' size={'12'}>
              <RangePicker format='YYYY-MM-DD' disabledDate={isDateDisabled} onChange={handleDataChange}/>
            </Space>
          </div>
          <div className='guest'>
            <label className='payment-labels'>Number of Guests:</label>
            <br/>
            <input className='no-of-guest' placeholder='Number of Guest' type='number' min='1' max={maximumGuest} required onChange={(e)=>{
          ...
          <div className='name-phoneno'>
            ...
            <br/>
            <input type='text' className='full-name' placeholder='Name' required onChange={(e) => {setUserData((prev)=>({...prev, name:e.target.value}))}} minLength='3' />
            <br/>
            
            <label className='payment-labels'>Phone Number:</label>
            <br/>
            <input type='tel' placeholder='Number' maxLength='10' onChange={(e)=>{
