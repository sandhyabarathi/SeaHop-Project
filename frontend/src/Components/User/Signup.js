    ...
    <form onSubmit={submitHandler} encType='multipart/form-data' className='col-10 col-lg-5'>
        ...
        {["name","email","password","passwordConfirm","phoneNumber"].map((field)=>(
            <div className='form-group' key={field}>
                ...
                <input type={field.includes("password")?"password":"text"} id={`${field}_field`} className='form-control' name={field} value={user[field]} onChange={onChange}/>
            </div>
        ))}
        <button id='regitser_button' type='submit' className='loginbutton btn-block py-3'>REGISTER</button>
